...
这一步是「建表」，相当于在 Supabase 里准备好存放角色、团队仓库的「格子」。  
...
2. 点击 **「New query」**（新建查询）
3. 打开项目里的 **`supabase-schema-v2.sql`** 文件（在 `dnd-team-app` 文件夹里），按 **Ctrl+A** 全选，**Ctrl+C** 复制，粘贴到 Supabase 左侧的输入框里  
   （本表结构用于「角色按人保存 + 团队仓库按模组共享」，和当前应用一致）
4. 点击右下角 **「Run」**（运行）按钮
...
- 完成上述步骤并正确填写了环境变量后，**角色会按登录名存到 Supabase，团队仓库会按模组共享**，多人可一起使用、进行物品交换与统一管理。
