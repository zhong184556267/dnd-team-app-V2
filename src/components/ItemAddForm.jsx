...
import { BUFF_TYPES, getCategories, normalizeEffectCategory, parseDamageString } from '../data/buffTypes'
...
function entryToEffectModules(entry, proto) {
  ...
  // 若条目已有 effects（与 BUFF 相同的结构），优先从中还原，避免通过附注/数值猜测导致丢失（旧 4 大类规范化为 6 大类）
  ...
  if (acMatch) add('defense', 'ac_bonus', { value: parseInt(acMatch[1], 10) || 0 })
...
export default function ItemAddForm({ open, onClose, onSave, submitLabel = '确认加入', editEntry = null }) {
  ...
  const [effectModules, setEffectModules] = useState(() => [createEmptyModule()])
  ...
  useEffect(() => {
    ...
    if (editEntry) {
      ...
      if (proto && (proto.类型 === '近战武器' || proto.类型 === '远程武器' || proto.类型 === '枪械')) {
        ...
        const { traits, range } = parseWeaponNoteToTraits(editEntry?.附注 ?? proto?.附注 ?? '')
      ...
      setIntro('')
      setQty(1)
      setEffectModules([createEmptyModule()])
      setArmorFields({ isShield: false, baseAC: '', dexMode: 'full', dexCap: 2, strReq: '', stealth: '—', shieldBonus: '' })
      setWeaponDamage({ minus: '', plus: '', o1: '', o2: '', type: '', o3: '' })
      setWeaponTraits([])
      setWeaponRange('')
  ...
  useEffect(() => {
    ...
    if (proto && (proto.类型 === '近战武器' || proto.类型 === '远程武器' || proto.类型 === '枪械')) {
      ...
      const { traits, range } = parseWeaponNoteToTraits(proto.附注 ?? '')
      setWeaponTraits(traits)
      setWeaponRange(range)
  ...
  const handleSubmit = (e) => {
    ...
    else if (isWeapon) 附注 = buildWeaponNoteFromTraits(weaponTraits, weaponRange) || (proto?.附注 ?? '').trim()
    ...
    const entry = {
      ...
      ...(effectsForSave.length ? { effects: effectsForSave } : {}),
    }
    onSave(entry)
    onClose()
  }
  ...
  return (
    <>
      ...
      <div className="fixed inset-4 sm:inset-auto sm:left-1/2 sm:top-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 sm:max-w-3xl sm:w-full z-[201] overflow-x-hidden overflow-y-auto max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
        <form onSubmit={handleSubmit} className="space-y-2.5 p-3 bg-gray-800 rounded-xl border border-gray-600 min-w-0 max-w-full">
            ...
            {isEdit ? (
              ...
              <div className="flex flex-wrap gap-1.5 min-w-0">
                ...
                <select
                  ...
                  className={inputClass + ' h-8 flex-1 min-w-0 text-sm'}
                  disabled={!type}
                >
          ...
          {/* 武器/枪械：伤害（±可调）+ 词条（勾选，射程 XX/XX）+ 精通（下拉）+ 附魔效果 */}
            ...
            <div className="w-full rounded border border-gray-600 bg-gray-700/30 px-2 py-1.5 space-y-1.5">
              ...
              <div className="w-full pt-1.5 border-t border-gray-600/80">
                <div className="flex items-center justify-between mb-0.5">
                  <label className="block text-dnd-gold-light text-xs font-bold uppercase tracking-wider">附魔效果</label>
                  <button type="button" onClick={addModule} className="flex items-center gap-1 px-1.5 py-0.5 rounded border border-amber-500 text-amber-400 hover:bg-amber-500/20 text-xs font-medium">
                ...
                <div className="w-full space-y-0.5">
                  {effectModules.map((mod) => {
                    ...
                    const effectiveEffectType = effectTypeValid ? mod.effectType : (visibleEffects[0]?.key ?? effects[0]?.key ?? '')
                    ...
                    return (
                      <div key={mod.id} className="w-full min-w-0 rounded border border-gray-600 bg-gray-700/30 px-1 py-0.5">
                        <div className="flex flex-nowrap items-center gap-0.5 min-w-0 w-full">
                          <div className="flex-1 min-w-0 basis-0 flex items-center">
                            <select value={mod.category} onChange={(e) => { const newCat = e.target.value; const newEffects = BUFF_TYPES[newCat]?.effects ?? []; updateModule(mod.id, { category: newCat, effectType: newEffects[0]?.key ?? '' }) }} className={inputClass + ' h-6 text-xs w-full min-w-0 py-0 text-left pl-1.5 pr-5'} title="效果大类">
                              {getCategories().map((c) => (<option key={c.key} value={c.key}>{c.label}</option>))}
                            </select>
                          </div>
                          <div className="flex-1 min-w-0 basis-0 flex items-center">
                            <select value={effectiveEffectType} onChange={(e) => updateModule(mod.id, { effectType: e.target.value })} className={inputClass + ' h-6 text-xs w-full min-w-0 py-0 text-left pl-1.5 pr-5'} title="具体效果">
                              {visibleEffects.map((e) => (<option key={e.key} value={e.key}>{e.label}</option>))}
                            </select>
                          </div>
                          <div className="flex-1 min-w-0 basis-0 flex items-center justify-start">
                            {!collapsed ? <EffectValueEditor module={{ ...mod, effectType: effectiveEffectType }} onChange={(next) => updateModule(mod.id, next)} catData={catData} inline /> : null}
                          </div>
                          <div className="flex-1 min-w-0 basis-0 flex items-center justify-end gap-0.5">
                            <button type="button" onClick={() => removeModule(mod.id)} className="h-6 w-6 rounded border border-gray-600 text-gray-400 hover:bg-red-900/40 hover:text-red-400 hover:border-red-600 flex items-center justify-center shrink-0" title="删减" aria-label="删减"><Trash2 className="w-3.5 h-3.5" /></button>
                            <button type="button" onClick={() => updateModule(mod.id, { collapsed: !collapsed })} className="h-6 w-6 rounded border border-gray-600 text-gray-400 hover:bg-gray-600 flex items-center justify-center shrink-0" title={collapsed ? '展开' : '折叠'}>{collapsed ? <ChevronDown className="w-3 h-3" /> : <ChevronUp className="w-3 h-3" />}</button>
                          </div>
                        </div>
                        {!collapsed && complexValue && (
                          <div className="pt-1 border-t border-gray-600/80 mt-1">
                            <EffectValueEditor module={{ ...mod, effectType: effectiveEffectType }} onChange={(next) => updateModule(mod.id, next)} catData={catData} />
                          </div>
                        )}
                      </div>
                    )
          ...
          {/* 盔甲/衣服：紧凑显示基础AC、敏调、力量要求、隐匿 */}
          {isArmorOrClothing && (
            <div className="rounded border border-gray-600 bg-gray-700/30 px-2 py-1.5 space-y-1.5">
              <div className="flex items-center justify-between gap-2">
                <span className="text-dnd-gold-light text-xs font-bold uppercase tracking-wider">护甲/衣服属性</span>
              </div>
              {isShield ? (
                <div className="flex items-center gap-2">
                  <span className="text-dnd-text-muted text-xs">盾牌 AC</span>
                  <NumberStepper
                ...
                <div className="flex flex-wrap items-center gap-2 text-xs">
                  <div className="flex items-center gap-1 min-w-[7rem] flex-1">
                    <span className="text-dnd-text-muted shrink-0">基础AC</span>
                    <input
                      type="number"
                      min={0}
                      value={armorFields.baseAC}
                      onChange={(e) => setArmorFields((f) => ({ ...f, baseAC: e.target.value }))}
                      className={inputClass + ' h-7 text-xs flex-1'}
                      placeholder="10"
                    />
                  </div>
                  <div className="flex items-center gap-1 min-w-[7rem] flex-1">
                    <span className="text-dnd-text-muted shrink-0">敏捷上限</span>
                    <select
                      value={armorFields.dexMode === 'cap' ? 'cap2' : 'full'}
                      onChange={(e) => {
                        const v = e.target.value
                        if (v === 'cap2') {
                          setArmorFields((f) => ({ ...f, dexMode: 'cap', dexCap: 2 }))
                        } else {
                          setArmorFields((f) => ({ ...f, dexMode: 'full' }))
                        }
                      }}
                      className={inputClass + ' h-7 text-xs flex-1'}
                    >
                      <option value="full">敏调</option>
                      <option value="cap2">+2</option>
                    </select>
                  </div>
                  <div className="flex items-center gap-1 min-w-[7rem] flex-1">
                    <span className="text-dnd-text-muted shrink-0">力量</span>
                    <input
                      type="text"
                  ...
                  <div className="flex items-center gap-1 min-w-[7rem] flex-1">
                    <span className="text-dnd-text-muted shrink-0">隐匿</span>
                    <select
